...
zbx_srv_installation() {
    ...
    hash_zbx_srv_conf_php="4593fb3a32c509b0af6d3b75d4da61ba  /etc/zabbix/web/zabbix.conf.php"
...
$ZBX_SERVER_NAME = 'ZBX_SRV';

$IMAGE_FORMAT_DEFAULT = IMAGE_FORMAT_PNG;
EOF

    sed -i "s/.*dbuser.*/\$DB['USER']     = \'$mariadb_user';/g" /etc/zabbix/web/zabbix.conf.php
    sed -i "s/.*dbpass.*/\$DB['PASSWORD'] = \'$mariadb_passwd';/g" /etc/zabbix/web/zabbix.conf.php
    sed -i "s/.*ZBX_SRV.*/\$ZBX_SERVER_NAME = \Zabbix_server';/g" /etc/zabbix/web/zabbix.conf.php
 ...
 [[ $(md5sum /etc/zabbix/web/zabbix.conf.php) != $hash_zbx_srv_conf_php ]] && zbx_srv_conf_php
...
zbx_agent_provision() {
    zbx_agent_installation
    tomcat_installation
    #add start for API script
    /vagrant/add_agent.sh
}
